...
plotResults <- function(data, y_high = 6.5, n = 1, scaled = T, violin = F) {
  ...
  if (violin == F) {  
    if (scaled == T) {
      plot<-ggplot(data_plots, aes(x = value)) + geom_density(aes(group = variable,colour = variable, fill = variable, y=..scaled..),
